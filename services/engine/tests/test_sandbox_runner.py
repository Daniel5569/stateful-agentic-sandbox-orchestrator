...
async def test_run_sandboxed_reports_nonzero_exit_code(tmp_path: Path) -> None:
    result = await run_sandboxed('python -c "raise SystemExit(7)"', tmp_path, timeout_seconds=5)
...
async def test_run_sandboxed_enforces_timeout(tmp_path: Path) -> None:
    result = await run_sandboxed('python -c "import time;time.sleep(1)"', tmp_path, timeout_seconds=0)
